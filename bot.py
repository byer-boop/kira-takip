...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [[InlineKeyboardButton("Kiraci Ekle", callback_data="kiraci_ekle")],[InlineKeyboardButton("Kiracilari Listele", callback_data="listele")],[InlineKeyboardButton("Odeme Isaretle", callback_data="odeme")],[InlineKeyboardButton("Durum Raporu", callback_data="rapor")],[InlineKeyboardButton("Kiraci Ayarlari", callback_data="kiraci_ayar")],[InlineKeyboardButton("Borclu Listesi", callback_data="borclu_menu")]]
    await update.message.reply_text("Kira Takip Botuna Hos Geldiniz!\n\nNe yapmak istersiniz?", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def depozito_al(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text("Kiraci eklendi!\nAd: " + kiraci["ad"] + " " + kiraci["soyad"] + "\nDaire: " + kiraci["daire"] + "\nKira: " + str(int(kiraci["kira"])) + " GBP\nOdeme gunu: " + str(kiraci["odeme_gunu"]) + "\nDepozito: " + depozito_str, reply_markup=InlineKeyboardMarkup(keyboard))
        return ConversationHandler.END
    except ValueError:
...
async def listele(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not kiracılar:
        await query.edit_message_text("Henuz kiraci yok.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Ana Menu", callback_data="ana_menu")]]))
        return
    bugun = datetime.now()
    ...
    for k in kiracılar:
        ...
        dep = " | Dep: " + str(int(k.get("depozito", 0))) + " GBP" if k.get("depozito", 0) > 0 else ""
        mesaj = mesaj + k["ad"] + " " + k["soyad"] + " - Daire " + k["daire"] + "\n" + str(int(k["kira"])) + " GBP | " + str(k["odeme_gunu"]) + ". gun | " + durum + dep + "\n\n"
    await query.edit_message_text(mesaj, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Ana Menu", callback_data="ana_menu")]]))
async def odeme_listesi(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text("Odeme Durumu:", reply_markup=InlineKeyboardMarkup(keyboard))
async def odeme_toggle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for k in data["kiracılar"]:
        if k["id"] == kiraci_id:
            if ay_key in k.get("odendi_aylar", []):
                ...
                mesaj = k["ad"] + " " + k["soyad"] + " - odeme isaretlendi"
            break
    veri_kaydet(data)
    keyboard = []
    for k in data["kiracılar"]:
    ...
    keyboard.append([InlineKeyboardButton("Kismi Odeme Ekle", callback_data="kismi_odeme")])
    keyboard.append([InlineKeyboardButton("Ana Menu", callback_data="ana_menu")])
    await query.edit_message_text("Odeme Durumu\n" + mesaj, reply_markup=InlineKeyboardMarkup(keyboard))
...
async def kismi_kiraci_sec(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    context.user_data["kismi_kiraci_id"] = query.data.replace("kismi_sec_", "")
...
async def kismi_miktar_al(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        for k in data["kiracılar"]:
            if k["id"] == kiraci_id:
                if "kismi_odemeler" not in k:
                    k["kismi_odemeler"] = {}
                k["kismi_odemeler"][ay_key] = k["kismi_odemeler"].get(ay_key, 0) + miktar
                ad = k["ad"] + " " + k["soyad"]
        ...
        veri_kaydet(data)
        keyboard = [[InlineKeyboardButton("Ana Menu", callback_data="ana_menu")]]
        await update.message.reply_text("Kismi odeme eklendi!\n" + ad + "\nBu ay odenen: " + str(int(toplam)) + " / " + str(int(kira)) + " GBP\nKalan: " + str(int(kira - toplam)) + " GBP", reply_markup=InlineKeyboardMarkup(keyboard))
        return ConversationHandler.END
    except ValueError:
...
async def rapor(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    mesaj = "Rapor - " + bugun.strftime("%B %Y") + "\n\nToplam kiraci: " + str(toplam) + "\nTam odeme: " + str(odenenler) + "\nGecikmiş: " + str(gecikmis) + "\n\nBeklenen: " + str(int(toplam_kira)) + " GBP\nToplanan: " + str(int(toplanan)) + " GBP\nKismi: " + str(int(kismi_toplam)) + " GBP\nBekleyen: " + str(int(toplam_kira - toplanan - kismi_toplam)) + " GBP"
    await query.edit_message_text(mesaj, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Ana Menu", callback_data="ana_menu")]]))
async def kiraci_ayar_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    keyboard = [[InlineKeyboardButton("Kiraci Sil", callback_data="kiraci_sil_listele")],[InlineKeyboardButton("Kira Zammi", callback_data="kira_zam_listele")],[InlineKeyboardButton("Isim Degistir", callback_data="isim_degistir_listele")],[InlineKeyboardButton("Ana Menu", callback_data="ana_menu")]]
...
async def kiraci_sil(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    veri_kaydet(data)
    await query.edit_message_text(silinen_ad + " silindi.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Ana Menu", callback_data="ana_menu")]]))
    ...
    try:
        ...
        for k in data["kiracılar"]:
            if k["id"] == kiraci_id:
                ...
                break
        veri_kaydet(data)
        await update.message.reply_text(ad + " kira guncellendi!\n" + str(int(eski)) + " GBP -> " + str(int(yeni_kira)) + " GBP", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Ana Menu", callback_data="ana_menu")]]))
        return ConversationHandler.END
    except ValueError:
        await update.message.reply_text("Gecerli bir sayi yazin:")
        return ZAM_MIKTAR
...
async def isim_kiraci_sec(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    context.user_data["isim_kiraci_id"] = query.data.replace("isim_sec_", "")
...
async def isim_yeni_al(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for k in data["kiracılar"]:
        if k["id"] == kiraci_id:
            ...
            break
    veri_kaydet(data)
    await update.message.reply_text(eski + " -> " + update.message.text + " olarak guncellendi.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Ana Menu", callback_data="ana_menu")]]))
    return ConversationHandler.END
async def borclu_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    keyboard = [[InlineKeyboardButton("Borclu Ekle", callback_data="borc_ekle")],[InlineKeyboardButton("Borclulari Listele", callback_data="borc_listele")],[InlineKeyboardButton("Odeme Isaretle", callback_data="borc_odeme")],[InlineKeyboardButton("Ana Menu", callback_data="ana_menu")]]
...
async def borc_tarih_al(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        borc_kaydet(data)
        await update.message.reply_text("Borclu eklendi!\nAd: " + borc["ad"] + "\nMiktar: " + str(int(borc["miktar"])) + " GBP\nVade: " + borc["tarih"], reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Ana Menu", callback_data="ana_menu")]]))
        return ConversationHandler.END
    except ValueError:
...
async def borc_listele(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not borclar:
        await query.edit_message_text("Henuz borclu yok.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Ana Menu", callback_data="ana_menu")]]))
        return
    bugun = datetime.now()
    ...
    for b in borclar:
        ...
        try:
            ...
            if not b["odendi"] and vade < bugun:
                durum = "GECIKTI (" + str((bugun - vade).days) + " gun)"
        except:
            pass
...
async def borc_odeme_listesi(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = []
    for b in borclar:
        etiket = ("[OK] " if b["odendi"] else "[BEKLIYOR] ") + b["ad"] + " - " + str(int(b["miktar"])) + " GBP"
        keyboard.append([InlineKeyboardButton(etiket, callback_data="borc_toggle_" + b["id"])])
    keyboard.append([InlineKeyboardButton("Ana Menu", callback_data="ana_menu")])
    await query.edit_message_text("Borclu Odeme Durumu:", reply_markup=InlineKeyboardMarkup(keyboard))
async def borc_toggle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for b in data["borclar"]:
        if b["id"] == borc_id:
            ...
            mesaj = b["ad"] + " - " + ("odendi" if b["odendi"] else "geri alindi")
            break
    borc_kaydet(data)
    keyboard = []
    for b in data["borclar"]:
...
def main():
    ...
    kiraci_conv = ConversationHandler(entry_points=[CallbackQueryHandler(kiraci_ekle_baslat, pattern="^kiraci_ekle$")], states={AD: [MessageHandler(filters.TEXT & ~filters.COMMAND, ad_al)], SOYAD: [MessageHandler(filters.TEXT & ~filters.COMMAND, soyad_al)], DAIRE: [MessageHandler(filters.TEXT & ~filters.COMMAND, daire_al)], KIRA: [MessageHandler(filters.TEXT & ~filters.COMMAND, kira_al)], TARIH: [MessageHandler(filters.TEXT & ~filters.COMMAND, tarih_al)], DEPOZITO: [MessageHandler(filters.TEXT & ~filters.COMMAND, depozito_al)]}, fallbacks=[CommandHandler("iptal", iptal)])
    borc_conv = ConversationHandler(entry_points=[CallbackQueryHandler(borc_ekle_baslat, pattern="^borc_ekle$")], states={BORC_AD: [MessageHandler(filters.TEXT & ~filters.COMMAND, borc_ad_al)], BORC_MIKTAR: [MessageHandler(filters.TEXT & ~filters.COMMAND, borc_miktar_al)], BORC_TARIH: [MessageHandler(filters.TEXT & ~filters.COMMAND, borc_tarih_al)]}, fallbacks=[CommandHandler("iptal", iptal)])
    zam_conv = ConversationHandler(entry_points=[CallbackQueryHandler(kira_zam_listele, pattern="^kira_zam_listele$")], states={ZAM_ID: [CallbackQueryHandler(zam_kiraci_sec, pattern="^zam_sec_")], ZAM_MIKTAR: [MessageHandler(filters.TEXT & ~filters.COMMAND, zam_miktar_al)]}, fallbacks=[CommandHandler("iptal", iptal)])
    isim_conv = ConversationHandler(entry_points=[CallbackQueryHandler(isim_degistir_listele, pattern="^isim_degistir_listele$")], states={ISIM_ID: [CallbackQueryHandler(isim_kiraci_sec, pattern="^isim_sec_")], ISIM_YENI: [MessageHandler(filters.TEXT & ~filters.COMMAND, isim_yeni_al)]}, fallbacks=[CommandHandler("iptal", iptal)])
    kismi_conv = ConversationHandler(entry_points=[CallbackQueryHandler(kismi_odeme_baslat, pattern="^kismi_odeme$")], states={KISMI_ID: [CallbackQueryHandler(kismi_kiraci_sec, pattern="^kismi_sec_")], KISMI_MIKTAR: [MessageHandler(filters.TEXT & ~filters.COMMAND, kismi_miktar_al)]}, fallbacks=[CommandHandler("iptal", iptal)])
